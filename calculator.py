...
class ManualBigNumber:
    ...
    def triangular_number_gmpy2_division(self, n: str):
        ...
        print ("Finished converting string file to pmpy2 format: time elapsed in method: " + str(f_convert_time))
        
        triangular = (n * (n + 1)) // 2
        gmpy2_calc_time = str(time.perf_counter() - start_time_gmpy2)
        print ("Finished calculating triangular number: elapsed in method: " + str(gmpy2_calc_time))
        return triangular
...
class TriangulaNumberMatrix:
    ...
    def pad_with_patterns(self, length: int, leftPattern: str, rightPattern: str, constant: str, rep_digit: str) -> str:
        ...
        print("building right pattern complete: " + str(time.perf_counter() - start_time_pad))

        left_pad = build_left(leftPattern, length)
        print("building left pattern complete: " + str(time.perf_counter() - start_time_pad))

        print("Right pad complete: " + str(time.perf_counter() - start_time_pad))
    ...
    def repDigitTriangularNumber(self, rep_digit: 'ManualBigNumber') -> 'ManualBigNumber':
        ...
        print ("padding complete: " + str(time.perf_counter() - start_time ));
    ...
    def pad_with_patterns_stream(
        ...
        written += w_left
        print("building left pattern complete (stream): " + str(time.perf_counter() - t0))
        ...
        written += w_right
        print("building right pattern complete (stream): " + str(time.perf_counter() - t1))

        print("pad_with_patterns_stream complete: " + str(time.perf_counter() - start_time_pad))
        return written
    ...
    def repDigitTriangularNumberStream(
        ...
        *,
        ...
        print("rep digit reducer complete (stream): " + str(time.perf_counter() - start_time))

        length = len(rep_str) - 2
        print("length operation complete (stream): " + str(time.perf_counter() - start_time))

        rep_char = rep_str[0]

        leftPattern = self.matrix.get(rep_char, {}).get("hpl", "")
        print("got leftPattern complete (stream): " + str(time.perf_counter() - start_time))

        rightPattern = self.matrix.get(rep_char, {}).get("hpr", "")
        print("got rightPattern complete (stream): " + str(time.perf_counter() - start_time))

        constant = self.matrix.get(rep_char, {}).get("vpc" + str(reduced), "0")
        print("got constant complete (stream): " + str(time.perf_counter() - start_time))
        ...
        print("stream write complete: " + str(time.perf_counter() - t_write))
        elapsed = time.perf_counter() - start_time
        print("repDigitTriangularNumberStream complete: " + str(elapsed))
        ...
        *,
        ...
        for ch in self.build_left_memory(leftPattern, length, chunk_chars=chunk_chars):
            if on_chunk:
                on_chunk(ch, offset)
            offset += len(ch)
        print("building left pattern complete (memory-chunks): " + str(time.perf_counter() - t_left))
        ...
        for ch in self.build_right_memory(rightPattern, right_len, chunk_chars=chunk_chars):
            if on_chunk:
                on_chunk(ch, offset)
            offset += len(ch)
        print("building right pattern complete (memory-chunks): " + str(time.perf_counter() - t_right))

        print("pad_with_patterns_memory complete: " + str(time.perf_counter() - t0))
    ...
    def repDigitTriangularNumberMemory(
        ...
        *,
        ...
        reduced = self.digit_reducer(rep_str, 4)
        print("rep digit reducer complete (memory-chunks): " + str(time.perf_counter() - start_time))
        ...
        print("repDigitTriangularNumberMemory complete: " + str(out["elapsed_seconds"]))
        return out
